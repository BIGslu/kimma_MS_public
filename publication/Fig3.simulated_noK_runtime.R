...
group.letter1 <- data.frame(multcompLetters(tukey.result1$model[,4])$Letters,
  ...
  mutate(let_new = recode(let_num,
                          "1"="a","2"="ab","3"="b","4"="c","5"="cd","6"="d","7"="e"))

p1a_signif <- p1a +
  geom_text(data = group.letter1, 
            aes(label = let_new, fill = processors), #Need fill to match orig plot groups
            vjust=0, size=3,
...
#### Save ####
p_all <- p1a_signif + p1b_signif + plot_annotation(tag_levels = "A")
# p_all

ggsave(filename="figs/Fig3.simulated_noK_runtime.pdf",
