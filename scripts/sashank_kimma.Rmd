...
#load data 
load("~/Documents/GitHub/kimma_MS_public/data/RSTR_RNAseq_data_combined_uniqueFULLID_weights.RData")
load("~/Documents/GitHub/kimma_MS_public/data/RSTR_RNAseq_kinship.RData")

#subset voom for LTBI vs RSTR
dat_LTBI_vs_RSTR <- dat.combined.voom

dat_LTBI_vs_RSTR$targets <- dat.combined.voom$targets %>% 
...
LTBI_vs_RSTR_vector <- dat.combined.voom$targets$libID %in% dat_LTBI_vs_RSTR$targets$libID
dat_LTBI_vs_RSTR$weights <- dat_LTBI_vs_RSTR$weights[, LTBI_vs_RSTR_vector]

#subset voom for media vs tb
dat_media_vs_tb <- dat.combined.voom

dat_media_vs_tb$targets <- dat.combined.voom$targets %>% 
...
media_vs_tb_vector <- dat.combined.voom$targets$libID %in% dat_media_vs_tb$targets$libID
...
#### Limma
#Add weights 
dat_LTBI_vs_RSTR$weights <- dat.combined.voom$weights[, LTBI_vs_RSTR_vector]
...
fit <- lmFit(object = dat_LTBI_vs_RSTR, 
             ...
             run.lmekin = TRUE)
efit <- eBayes(fit)
results_limma_LTBI_vs_RSTR_kinship <- kimma::extract_lmFit(design = mm_LTBI_vs_RSTR_limma, fit=efit)
summarise_lmFit(results_limma_LTBI_vs_RSTR_kinship)
save(efit, results_limma_LTBI_vs_RSTR_kinship, file = "Sample_Group_limma_kinship")
...
fit <- kmFit(dat = dat_LTBI_vs_RSTR, 
             ...
             run.lmekin = TRUE, 
             use.weights = FALSE)
results_kimma_LTBI_vs_RSTR_kinship <- fit$lm
save(fit, results_kimma_LTBI_vs_RSTR_kinship, file = "Sample_Group_kimma_kinship")
...
fit <- lmFit(
  object = dat_LTBI_vs_RSTR, 
  design = mm_LTBI_vs_RSTR_limma, 
  weights = dat_LTBI_vs_RSTR$weights, 
  kin = kin.matrix, 
  run.lmekin = TRUE)
efit <- eBayes(fit)
results_limma_LTBI_vs_RSTR_kinship_weights <- kimma::extract_lmFit(design = mm_LTBI_vs_RSTR_limma, fit = efit)
save(efit, results_limma_LTBI_vs_RSTR_kinship_weights, file = "Sample_Group_limma_kinship_weights")
...
fit <- kmFit(dat = dat_LTBI_vs_RSTR,
             ...
             run.lmekin = TRUE
             use.weights = TRUE)
results_kimma_LTBI_vs_RSTR_kinship_weights <- fit$lm
save(fit, results_kimma_LTBI_vs_RSTR_kinship_weights, file = "Sample_Group_kimma_kinship_weights")
...
#### Kimma 
fit <- kmFit(dat = dat_media_vs_tb, model = "~ condition" , run.lm = TRUE, patientID = "FULLIDNO")
...
#### Limma
#Add weights 
dat_media_vs_tb$weights <- dat.combined.voom$weights[, media_vs_tb_vector]
